export const replaceAt = (s: string, replacement: string, i: number) => s.substring(0, i) + replacement + s.substring(i+1)

export const animate = ({ speed, initialPosition }: { speed: number, initialPosition: string}): string[] => {
    const steps: string[] = [initialPosition.replace(/[RL]/g, 'X')];
    const { length } = initialPosition;
    const finalString = '.'.repeat(length)
    const particles = [...initialPosition.matchAll(/[RL]/g)].map(particle => ({ direction: particle[0], index: particle.index }));
